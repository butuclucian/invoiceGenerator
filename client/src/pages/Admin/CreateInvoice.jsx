...
const CreateInvoice = () => {
  ...
  invoice_number: "Loading...", // Placeholder până se încarcă numărul corect
  ...
  const initializeForm = async () => {
    try {
      ...
      if (location.state?.aiData) {
        ...
        const tax_amount = (subtotal * defaultTax) / 100;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData._aiPendingId) {
        await API.put(`/invoices/${formData._aiPendingId}/approve`, formData);
      ...
      <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-12">
          ...
          {formData.items.map((item, index) => (
            <div key={index} className="grid md:grid-cols-12 gap-4 items-end p-4 bg-[#1a1a1a]/70 border border-white/10 rounded-lg mb-3">
              ...
              <div className="md:col-span-2">
                ...
                <input readOnly value={`${item.total.toFixed(2)} ${currency}`} className="w-full bg-transparent border border-white/10 rounded-md px-3 py-2 text-gray-400"/>
              </div>
        ...
        <section>
          ...
          <div className="mt-4 space-y-2 text-gray-300 border-t border-white/10 pt-4">
            <div className="flex justify-between">
              <span>Subtotal:</span>
              <span>{formData.subtotal.toFixed(2)} {currency}</span>
            </div>

            {formData.discount_amount > 0 && (
              <div className="flex justify-between text-red-400">
                <span>Discount ({formData.discount_rate}%):</span>
                <span>- {formData.discount_amount.toFixed(2)} {currency}</span>
              </div>
            )}
            ...
            {formData.tax_amount > 0 && (
              <div className="flex justify-between text-[#80FFF9]">
                ...
                <span>+ {formData.tax_amount.toFixed(2)} {currency}</span>
              </div>
            )}
        ...
        <div className="fixed bottom-0 right-0 left-0 md:left-64 bg-[#111111]/90 border-t border-white/10 backdrop-blur-md py-3 z-10">
          <div className="flex flex-row justify-center items-center gap-2 sm:gap-4 px-3 sm:px-4">
            ...
            <button type="submit" disabled={loading} className="flex items-center justify-center gap-2 px-3 sm:px-5 py-2 rounded-md bg-indigo-600/20 border border-indigo-600/40 hover:bg-indigo-600/30 transition text-sm sm:text-base" >
              <Save size={16} />
