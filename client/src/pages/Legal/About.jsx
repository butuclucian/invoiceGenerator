    ...
    <div className="min-h-screen bg-[#E8E8E8] pt-32 overflow-hidden selection:bg-[#1E1E1E] selection:text-[#E8E8E8]">
      ...
      <div className="max-w-[1700px] mx-auto">
        ...
        <div className="flex flex-col mb-16 relative">
          <motion.div  initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.215, 0.61, 0.355, 1.0] }} className="w-full font-bold text-[80px] md:text-[150px] lg:text-[200px] tracking-[-9%] leading-[0.8] uppercase select-none">
        ...
        <motion.div  variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-12 gap-6">
          {/* Card 1: The Mission (Dark & Massive) */}
          <motion.div  variants={itemVariants} className="md:col-span-8 bg-[#1E1E1E] text-[#E8E8E8] rounded-[2.5rem] p-10 md:p-16 flex flex-col justify-between relative overflow-hidden group shadow-2xl">
            {/* Element grafic decorativ */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full blur-3xl transform translate-x-1/2 -translate-y-1/2"></div>
            
            <span className="text-sm font-bold uppercase tracking-[0.2em] text-gray-400 mb-12 block">
              // 01 The Mission
            </span>
            <h2 className="text-4xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-[1.1]">
              We were built to <span className="text-transparent [-webkit-text-stroke:1px_#E8E8E8]">simplify invoicing</span> for the modern digital business.
            </h2>
          </motion.div>

          {/* Card 2: The Tech (Outlined Minimalist) */}
          <motion.div  variants={itemVariants} className="md:col-span-4 border-2 border-[#1E1E1E] bg-transparent text-[#1E1E1E] rounded-[2.5rem] p-10 flex flex-col justify-between group">
            <span className="text-sm font-bold uppercase tracking-[0.2em] mb-8 block transition-colors duration-500">
              // 02 The Tech
            </span>
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold tracking-tight leading-snug">
                Our platform uses advanced AI to automate the entire billing workflow — from creation to data extraction, analytics, and smart reminders.
              </p>
            </div>
          </motion.div>

          {/* Card 3: The Philosophy (Accent Color) */}
          <motion.div  variants={itemVariants} className="md:col-span-12 bg-purple-500 text-[#1E1E1E] rounded-[2.5rem] p-10 md:p-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-10 shadow-xl relative overflow-hidden">
            {/* Grafică abstractă de fundal */}
            <motion.div  animate={{ rotate: 360 }} transition={{ duration: 50, repeat: Infinity, ease: "linear" }} className="absolute -right-20 -bottom-40 w-96 h-96 border border-[#1E1E1E]/20 rounded-full"/>
            <motion.div  animate={{ rotate: -360 }} transition={{ duration: 40, repeat: Infinity, ease: "linear" }} className="absolute -right-10 -bottom-20 w-64 h-64 border border-[#1E1E1E]/20 rounded-full"/>
            ...
            <div className="relative z-10 max-w-4xl">
              ...
              <p className="text-3xl md:text-5xl font-black tracking-tighter leading-[1.1]">
                We believe in clean design, absolute automation, and zero friction for professionals who value their time. Built to scale with you.
              </p>
            </div>
          </motion.div>
