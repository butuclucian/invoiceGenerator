...
import axios from 'axios'; // Sau instanța ta personalizată de API
...
const NotificationBell = () => {
  ...
  // Luăm URL-ul backend-ului din variabilele de mediu Docker
  const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000/api';

  const fetchAiNotifications = async () => {
    try {
      // Preluăm token-ul de autentificare (ajustează în funcție de cum îl salvezi: localStorage/cookies)
      ...
      // Filtrăm doar facturile care provin din email/AI și sunt în starea de așteptare ('pending')
      // De asemenea, le sortăm ca cele mai noi să fie primele
  ...
  useEffect(() => {
    ...
    // Verificăm automat la fiecare 15 secunde dacă a mai sosit un email
  ...
  const handleNotificationClick = (invoiceId) => {
    setIsOpen(false);
    // Îndrumăm utilizatorul către pagina facturii specifice sau către lista de revizuire
    navigate(`/invoices/${invoiceId}`); 
  };

  return (
    <div className="relative inline-block text-left">
      {/* Butonul Clopoțel cu Glassmorphism */}
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="relative p-2.5 text-gray-300 hover:text-white rounded-xl bg-white/5 border border-white/10 backdrop-blur-md transition-all duration-300 hover:bg-white/10"
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
          <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0" />
        </svg>

        {/* Bulina Roșie de alertă (Apare doar dacă sunt mesaje noi) */}
          ...
          <div className="max-h-64 overflow-y-auto mt-1 space-y-1">
            {notifications.length === 0 ? (
              ...
              notifications.map((inv) => (
                <button
                  key={inv._id}
                  onClick={() => handleNotificationClick(inv._id)}
                  className="w-full text-left p-3 rounded-xl hover:bg-white/5 transition-all duration-200 border border-transparent hover:border-white/5 flex flex-col gap-1"
                >
