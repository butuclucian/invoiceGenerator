...
const Sidebar = ({ isMobile = false, closeSidebar }) => {
  ...
  return (
    <motion.aside initial={{ x: isMobile ? "-100%" : 0 }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ duration: 0.35, ease: "easeOut" }} className={` fixed top-0 left-0  h-screen w-64  bg-[#111111]      /* FULL background */ border-r border-white/10  text-white  flex flex-col justify-between  shadow-xl shadow-black/40  z-50  ${isMobile ? "md:hidden" : ""} `} >
      {/*  Glow Effects */}
 ...
 >
        ...
        {openMenu.invoices && (
          <div className="ml-8 mt-1 flex flex-col border-l border-white/10 pl-3 relative z-20">
            <NavLink to="/dashboard/invoices/create" className={({ isActive }) => `flex items-center gap-2 px-2 py-2 rounded-md ${ isActive ? "text-[#80FFF9]" : "text-gray-400 hover:text-white" }` } >
              <FilePlus2 size={16} /> Create Invoice
            </NavLink>

            <NavLink to="/dashboard/invoices" className={({ isActive }) => `flex items-center gap-2 px-2 py-2 rounded-md ${ isActive ? "text-[#80FFF9]" : "text-gray-400 hover:text-white" }` } >
              <FileText size={16} /> All Invoices
            </NavLink>

            <NavLink to="/dashboard/invoices/recurring" className={({ isActive }) => `flex items-center gap-2 px-2 py-2 rounded-md ${ isActive ? "text-[#80FFF9]" : "text-gray-400 hover:text-white" }` } >
